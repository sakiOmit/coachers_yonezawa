...
### 6b. 1:N Heading-Content パターン（★ Issue #274 強化）
- **見出し/トピック要素**（TEXT型、またはテキスト中心の小フレーム）の直後に、関連する**複数の活動/詳細要素**が縦に続くパターン
- 見出し要素は通常: TEXT型、子要素少数、高さが小さい、テキスト内容がトピックを示す
- コンテンツ要素は通常: 同じ ChildTypes を持つ3+個の連続する FRAME/INSTANCE
- **1つのセットとしてグルーピング**する（見出し + 全コンテンツアイテムを同一グループ）

#### 具体例

**例1: CSR活動ページ**
```
| # | ID     | Type  | Text           | ChildTypes |
|---|--------|-------|----------------|------------|
| 1 | 2:100  | TEXT  | 気候変動対策     | -          |  ← 見出し
| 2 | 2:101  | FRAME | 太陽光パネル設置  | 1IMG+2TEX  |  ← コンテンツ1
| 3 | 2:102  | FRAME | 省エネ活動       | 1IMG+2TEX  |  ← コンテンツ2
| 4 | 2:103  | FRAME | 資源リサイクル    | 1IMG+2TEX  |  ← コンテンツ3
| 5 | 2:200  | TEXT  | 地域貢献活動      | -          |  ← 次の見出し
| 6 | 2:201  | FRAME | ボランティア参加  | 1IMG+2TEX  |  ← 次のコンテンツ1
| 7 | 2:202  | FRAME | 寄付活動         | 1IMG+2TEX  |  ← 次のコンテンツ2
```
→ グループ1: `climate-action` (heading-content) = [2:100, 2:101, 2:102, 2:103]
→ グループ2: `community-contribution` (heading-content) = [2:200, 2:201, 2:202]

**例2: 特徴紹介ページ**
```
| # | ID     | Type  | Text        | ChildTypes |
|---|--------|-------|-------------|------------|
| 1 | 3:10   | FRAME | FEATURES    | 1TEX       |  ← EN見出し
| 2 | 3:11   | FRAME | 私たちの特徴 | 1TEX       |  ← JP見出し
| 3 | 3:12   | FRAME | 高品質       | 2TEX+1VEC  |  ← 特徴1
| 4 | 3:13   | FRAME | 迅速対応     | 2TEX+1VEC  |  ← 特徴2
| 5 | 3:14   | FRAME | 低コスト     | 2TEX+1VEC  |  ← 特徴3
```
→ グループ: `features-area` (heading-content) = [3:10, 3:11, 3:12, 3:13, 3:14]

#### 判定ヒント
- **ChildTypes が同一の連続要素**が見出しの後に続く → 1:N パターンの可能性大
- Y座標が連続し、divider/区切り線で区切られた範囲内の要素を1セットとする
- **CSS実装: 1つの `<section>` or `<div>` としてコーディングされる単位**
- ⚠️ 見出しと活動を別々のグループにしない。見出し単独グループ + 活動単独グループではなく、セットで1グループ
