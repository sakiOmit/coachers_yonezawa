...
| フラット構造 | 直接子要素 > 15 | -5/件 + 超過子要素×0.5 (上限-30) |
...
**注意**: セマンティック検出（card, media-object 等）は Stage A から削除済み（Issue 29/30）。
セマンティック理解は Stage B（Claude 推論）に委ねる設計。
...
Stage A（ネストレベルのグルーピング）と Stage B（トップレベルのセクショニング）は
対象レベルが異なるため、基本的に重複しない。それぞれ独立して適用する。

- Stage A: proximity + pattern + spacing + semantic によるネストレベルのグルーピング候補
- Stage B: Claude 推論によるトップレベル children のセクション分割

### Stage A 検出メソッド

| メソッド | 優先度 | 説明 |
|---------|-------|------|
| semantic | 4 | 構造ベースのCard/Nav/Grid検出 + ヘッダー/フッター検出（fills非依存） |
...
- Card: FRAME/COMPONENT/INSTANCE、子2-6個、IMAGE/RECTANGLE + TEXT
- Navigation: 4+子要素、水平配置、各TEXT幅 < 200px
- Grid: 2+行 × 2+列、要素サイズ類似（20%以内）
