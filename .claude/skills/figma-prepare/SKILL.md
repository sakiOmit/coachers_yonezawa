...
#### 2-4b. sectioning-plan → grouping-plan 変換ロジック
...
#### 2-4d. apply-grouping.js 実行（各レベル）
...
Stage A のグルーピング候補を適用。Stage B 適用後に parent が変わっている可能性があるため、
各候補の parent_id を現在のノード親から再取得する。

```
使用手順:
1. scripts/apply-grouping.js を読み込み
2. __GROUPING_PLAN__ を Stage A 候補JSONに置換
