...
- **Stage C**: Claude ネストレベル推論（Haiku でセクション内部をグルーピング） **← 必須。スキップ禁止**
...
### 2-3. Stage C: Claude ネストレベル推論（Haiku） ← **必須ステップ。スキップ禁止。**

> **⚠️ このステップは必ず実行すること。** Stage B 完了後にここをスキップして Phase 3 に進んではならない。
> Stage C を実行しないと、セクション内部の構造化（2カラム分割、カード認識、装飾分離）が一切行われない。
...
#### 2-3e. Stage C 再帰的ネスト処理（Issue #224）

Stage C の初回推論結果で `pattern != "single"` **または `node_ids >= 3`** のグループに対して、そのグループ内部の children をさらに Stage C に投入する（Issue #255）。最大再帰深度は 2（セクション → グループ → サブグループ）。

```
再帰ループ:
  depth = 0 (初回: sectioning-plan のリーフセクション)

  WHILE depth < MAX_STAGE_C_DEPTH (2):
    depth += 1
    # Issue 255: pattern="single" でも children>=3 なら再帰対象
    recursion_targets = [g for g in current_groups if g.pattern != "single" or len(g.node_ids) >= 3]
    IF recursion_targets が空 → BREAK

    # 非single グループの children を取得
    bash generate-nested-grouping-context.sh \
      .claude/cache/figma/prepare-metadata-{nodeId}.json \
      .claude/cache/figma/sectioning-plan.yaml \
      --groups .claude/cache/figma/nested-grouping-plan-depth{depth-1}.yaml \
      --depth {depth} \
      --output .claude/cache/figma/nested-context-depth{depth}.json

    # 各セクション（= 非single グループ内部）で Haiku 推論
    FOR each section in nested-context-depth{depth}.json:
      IF section.total_children < 2 → SKIP
      Haiku 推論 → YAML パース・検証
    END

    # 結果を nested-grouping-plan-depth{depth}.yaml に保存
    current_groups = 新しい推論結果
  END

最終統合:
  nested-grouping-plan.yaml に全 depth の結果をマージ
  各 depth の groups を統合リストとして保持
```

**コスト影響**: depth 1 あたり $0.003-0.009（Haiku × 非single グループ数）。depth 2 まででも合計 $0.02 以下。

**スキップ条件**: グループの children 数が 2 未満の場合はスキップ（分割不要）。
...
#### 2-3f. Divider 吸収ポストプロセス（Issue #253）

Stage C 結果の `nested-grouping-plan.yaml` に対して、単一要素の divider グループを隣接する list-item グループに吸収する。
これにより `figma-implement` 時に divider が `border-bottom` として正しく解釈される。
...
4. [オプション] Stage C 高カバレッジ確認後:
   - 次回以降の同ページ実行時に --disable-detectors を使用
   - 例: --disable-detectors bg-content,table,highlight,tuple,consecutive,heading-content
   - 注意: STAGE_A_ONLY 検出器（header-footer, horizontal-bar, zone, semantic, proximity, spacing, pattern）は無効化不可
```
...
現在は手動で `--disable-detectors` を指定する。将来的には:
- compare-grouping の結果に基づいて自動で `--disable-detectors` フラグを生成
- SKILL.md ワークフローで Stage C 高カバレッジセクションの検出器を自動無効化

### 2-5. dry-run / --apply

> **⚠️ 前提条件チェック**: --apply 実行前に以下のファイルが存在することを確認せよ。
> 存在しない場合は Phase 2 の該当 Stage が未実行。Phase 3 に進まず、不足 Stage を実行すること。
>
> | ファイル | 生成元 | 必須 |
> |---------|--------|------|
...
Stage C の再帰結果（Issue #224）により nested-grouping-plan.yaml は複数の depth を持つ。
depth 0 から昇順に適用し、各 depth で生成されたラッパーIDを次の depth の parent_id として使用する。
...
2. depth 0 から昇順に適用（外→内）:
   FOR depth = 0 TO max_depth:
     ...
     b. parent_id を現在のノード親から動的取得
        ...
        ※ depth > 0: 前 depth で作成されたラッパーが新しい parent になっている
           → 前 depth の apply-grouping.js 出力 wrappers[].id から取得
     c. apply-grouping.js で実行 → 新ラッパーIDを記録
     d. 次 depth の parent_id マッピングを更新

3. Stage A フォールバックセクションは depth 0 のみ
   ※ Stage A にフォールバックしたセクションの内部は再帰 Stage C 対象外

4. verify-grouping.js で全 depth のラッパーを検証
   ※ __VERIFICATION_PLAN__ に depth 0 〜 max_depth の全ラッパーを含める
```
...
5. Stage A / Stage C のネストレベルグルーピングを depth 0 から適用（2-5e 参照）
6. Stage C 再帰ネストの適用（depth 1+）
   ※ nested-grouping-plan に depth > 0 のエントリがある場合:
     - depth 0 適用後に生成されたラッパーIDを parent_id として使用
     - depth 1 → depth 2 の順にトップダウンで適用
     - 各 depth 適用後に verify-grouping.js で部分検証
