...
### Issue 90: Phase 2-4 の計画をFigmaに適用する `--apply` 機能がない
- **Phase**: 全体
- **優先度**: 高（figma-to-code精度の肝）
- **概要**: 現在のPhase 2（グルーピング）、Phase 3（リネーム）、Phase 4（Auto Layout）はYAML計画の出力のみ。Figma Plugin APIを使った実際の適用機能が未実装。
- **必要なスクリプト**:
  - `scripts/apply-renames.sh` — Phase 3のYAMLを読み、`figma.getNodeById(id).name = newName` を実行
  - `scripts/apply-grouping.sh` — Phase 2のYAMLを読み、ラッパーFrame作成 + 子要素移動（bounding box計算、z-order維持）
  - `scripts/apply-autolayout.sh` — Phase 4のYAMLを読み、`layoutMode/itemSpacing/padding/counterAxisAlignItems` を設定
- **実行方法**: Chrome DevTools MCP経由で `evaluate_script` を使いFigma Plugin APIを呼び出す
- **安全策**: バッチ50件/回、ID存在チェック必須、Figmaブランチ上での実行推奨
- **検証済み**: 手動で38:718（base page）に適用し、40要素→15要素への構造化に成功。ビジュアル崩れなし。

### Issue 91: グルーピング計画のセクション名がセマンティックでない
- **Phase**: 2
- **優先度**: 中
- **概要**: Phase 2のzone検出で生成されるグループ名が汎用的（`section` のみ）。実際の適用時に手動で `section-hero`, `section-menu-1` 等に命名する必要があった。Phase 3のリネームロジックと連携して、グルーピング時点でセマンティックな名前を推論すべき。
- **例**: `section` → `section-menu-1`（子要素に4つのカードフレームがある場合）、`section` → `section-hero`（背景RECTANGLE + テキストFRAMEの場合）

## 改善方針

### 品質基準

- フィクスチャテスト: 全件パス（回帰防止）— 85件
...
| 90 | 全体 | **OPEN** | Phase 2-4の計画をFigmaに適用する `--apply` 機能がない — apply-renames.sh / apply-grouping.sh / apply-autolayout.sh の新設が必要。Chrome DevTools MCP経由でFigma Plugin API実行。手動検証で40→15要素構造化に成功済み |
| 91 | 2 | **OPEN** | グルーピング計画のセクション名が汎用的（`section` のみ） — zone検出時にセマンティックな名前を推論すべき（子要素構成からhero/menu/features等を判定） |
