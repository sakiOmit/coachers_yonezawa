...
| Issue | Phase | 重要度 | 概要 |
| ----- | ----- | ------ | ---- |
| 69 | 4 | Low | `infer-autolayout.sh` が INSTANCE/COMPONENT ノードの Auto Layout 推論をスキップ |
| 70 | 4 | Low | `layout_from_enrichment` の `source` が base metadata でも `'enriched'` と表示される |
| 71 | 1 | Low | `analyze-structure.sh` の `max_depth` が絶対深度で報告されるがスコアは相対深度を使用 |
| 72 | 1 | Low-Med | `analyze-structure.sh` が INSTANCE 型ノードをフラット/深ネスト指標に含めない |
| 73 | docs | Low | SKILL.md Usage/Input Parameters に `--enrich` フラグ未記載 |

## 改善方針

### 品質基準

- フィクスチャテスト: 全件パス（回帰防止）— 82件
...
- figma_utils.py ユニットテスト: 全 public 関数カバー（9関数）
