...
### Issue 24: `resolve_absolute_coords` が5スクリプトに重複定義されている

- **Phase**: 全体
- **優先度**: 低
- **概要**: 同一の `resolve_absolute_coords` 関数が以下の5スクリプトに重複して定義されている:
  - `analyze-structure.sh`
  - `detect-grouping-candidates.sh`
  - `generate-rename-map.sh`
  - `infer-autolayout.sh`
  - `prepare-sectioning-context.sh`
- **現状**: 全実装が同一のため機能的問題はないが、修正時に5箇所同時更新が必要
- **対応方針**: 共通 Python ライブラリに切り出し、各スクリプトから import する構成を検討
- **リスク**: 低（現状で不整合なし、保守性の改善のみ）
...
- スクリプト共通関数の Python ライブラリ化（Issue 24）
...
| 24 | 全体 | **OPEN** | `resolve_absolute_coords` の5スクリプト重複定義 |
