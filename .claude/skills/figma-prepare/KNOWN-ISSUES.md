...
| # | Phase | 概要 | Status |
|---|-------|------|--------|
| 272 | 4 | resolve_absolute_coords() がXMLメタデータの絶対座標を二重加算 — `_coords_artboard_relative`フラグで検出し、ルートオフセットのみ加算する`_resolve_artboard_relative()`を追加。parse_figma_xml()でフラグ自動設定 | DONE |
| 273 | 2 | build_pattern_registry() がroot_children(リスト)を受け取ると .get() エラー — isinstance(root, list)チェック追加、リストを合成ルートにラップ。hiddenフィルタ付き。テスト4件追加(17全パス) | DONE |
| 274 | 2 | 1:N heading-content グルーピング未対応 — grouping_llm_fallback.py新規モジュール作成。enriched tableベースで見出し→N件コンテンツパターンを検出、LLMにグルーピング修正を提案させる。rename_llm_fallbackと対称的設計。プロンプトテンプレート・テスト44件追加 | DONE |
