...
### Issue 25: `get_bbox` の返却キー名が `prepare-sectioning-context.sh` のみ異なる

- **Phase**: 全体
- **優先度**: 低
- **概要**: `get_bbox` 関数の返却キー名がスクリプト間で不整合:
  - `detect-grouping-candidates.sh`, `infer-autolayout.sh`, `lib/figma_utils.py`: `{x, y, w, h}`
  - `prepare-sectioning-context.sh`: `{x, y, width, height}`
- **現状**: `prepare-sectioning-context.sh` の出力 JSON に `width`/`height` キーが含まれるため、
  出力フォーマットの後方互換性のためにローカル定義を残している。機能的問題はない。
- **対応方針**: 出力フォーマットを `w`/`h` に統一し、下流消費側（Stage B の Claude 推論等）を
  同時に更新する。優先度は低い。
...
| 25 | 全体 | **OPEN** | `get_bbox` 返却キー名の不整合（prepare-sectioning-context.sh のみ width/height） |
