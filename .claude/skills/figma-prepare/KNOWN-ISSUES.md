...
### Issue 92: apply-grouping.js の適用結果を検証する verify-grouping.js がない
- **Phase**: 2
- **優先度**: 中
- **概要**: `apply-renames.js` には対応する `verify-structure.js` があるが、`apply-grouping.js` の適用後にラッパーFRAMEが正しく作成され子要素が移動されたかを検証するスクリプトがない。
- **必要な検証項目**:
  - ラッパーFRAMEのname / 子要素数
  - 子要素が正しく移動されたか（node_idsの存在チェック）
  - bounding boxが元の配置と一致するか（ビジュアル崩れなし）

### ~~Issue 93: SKILL.md の Phase 2/4 `--apply` セクションが新スクリプト名を参照していない~~ → FIXED
- **Phase**: docs
- **状態**: FIXED — SKILL.md Phase 2-4 / Phase 4-2 に `apply-grouping.js` / `apply-autolayout.js` のスクリプト名・プレースホルダー置換手順を追記
...
| 92 | 2 | **OPEN** | `apply-grouping.js` の適用後検証スクリプト `verify-grouping.js` がない — ラッパーFrame・子要素移動・bbox一致の検証 |
