...
**提案**:
1. `detect_bg_content_layers()` の bg 候補判定を拡張: width > page_width も bg 候補に含める（既存は parent_width の 80% のみ）
2. `infer_zone_semantic_name()` で width > page_width の RECTANGLE/IMAGE を `has_large_bg` として扱う
3. x < 0 の要素も「はみ出し背景」として bg_candidates に追加
4. 新定数: `OVERFLOW_BG_MIN_WIDTH = 1400` (px) — 幅がページ幅に近い/超過する要素を背景候補とする
...
**提案**:
1. `detect_horizontal_bar()` を Stage A に追加:
   - 兄弟要素のうち Y 範囲が狭い帯域（高さ < 100px）に 4+ 要素が集中
   - 帯域内に 1 つ以上の背景 RECTANGLE が存在
   - 要素が水平に分布（X 分散 > Y 分散 × 3）
2. 既存の zone 検出はこの帯域を 1 zone として検出するはずだが、zone 全体が狭すぎて他の zone にマージされるリスクがある
3. 帯域内の RECTANGLE を bg、残りを content として `news-bar` 命名

**検証方法**: 上記6要素が 1 つのグルーピング候補（`suggested_name: 'news-bar'` または類似）として検出されること
...
**Phase**: 1
**状態**: OPEN
**概要**: トップ_0510修正は約85個の直接子要素を持つ超フラット構造。現在のスコアリングでは `flat_sections × 5 + flat_excess × 0.5` でペナルティを計算するが、85 子要素の場合:
- `flat_sections = 1`（ルート自体）
- `flat_excess = 85 - 15 = 70`
- `flat_penalty = min(30, 1 × 5 + 70 × 0.5) = min(30, 40) = 30`（上限に達する）

flat_penalty が上限30に達してしまい、子要素85個と子要素50個を区別できない。一方 unnamed_rate は85要素中の多くが「Rectangle N」「Group N」「Text ...」パターンなので高くなり、unnamed_penalty も上限30に近づく可能性がある。結果としてスコアが過度に低くなり、構造的に整理済み（既にセマンティック名を持つフレームが混在）の要素まで過小評価される。

**発見箇所**: トップ_0510修正 (2:8315) — 85 direct children
試算:
- 85 children のうち既に命名済み（`赤パネル`、`キャッチコピー`、`header`、`会社情報`、`事業紹介` 等）が ~15 要素
- 未命名（`Rectangle *`、`Group *`、`Frame *`、`AdobeStock_*`、`Text *` 等）が ~60 要素
- unnamed_rate ≈ 60/total × 100 → unnamed_penalty: 高い
- flat_penalty: 30（上限）
- total penalty ≈ 55-65 → score ≈ 35-45 (Grade D-C)

**提案**:
1. flat_penalty の上限を 30 → 40 に引き上げ検討（超フラットの差別化）
2. 代替: `flat_excess` のウェイトを非線形にする（例: `flat_excess^0.7 × 0.8`）
3. キャリブレーション: ABOUT ページ（40 children）とトップページ（85 children）を含む比較データセットでスコア範囲を検証
4. 命名済み要素の比率を追加指標として報告（`named_rate_pct`）— 構造は悪いが命名は良いケースを区別

**検証方法**: キャリブレーションデータセットにトップ_0510修正を追加し、Grade C (40-59) に収まることを確認。ABOUT ページとのスコア差が構造差を反映していることを検証
...
**提案**:
1. `is_section_root()` の幅判定を緩和: `abs(width - SECTION_ROOT_WIDTH) < 10` → `width >= SECTION_ROOT_WIDTH * 0.9` も section root として扱う（1296px 以上）
2. 代替: 幅超過要素は「装飾的はみ出し」として section root とは別枠で扱い、内部のグルーピング走査は通常通り実行
3. Stage A の `walk_and_detect()` がこの Group 内を再帰走査する際に、4つのサブ Group をセマンティック検出（contact/footer-links/cta-bar/sitemap）できるよう、footer 内部パターンの検出を強化
...
- フィクスチャテスト: 全件パス（回帰防止）— 357件(unit + integration)
...
- 残 OPEN Issue: 183 (oversized), 184 (ニュースバー), 188 (フラットスコア), 191 (フッターラッパー)
...
| 183 | 2 | **OPEN** | 親幅超過要素（oversized elements）のグルーピング誤分類 |
| 184 | 2 | **OPEN** | ニュースバー（横並び多要素フラットパターン）のグルーピング未検出 |
...
| 188 | 1 | **OPEN** | 超フラット構造（85+直接子要素）のスコア精度検証 |
...
| 191 | 2 | **OPEN** | フッター統合ラッパー（幅超過 Group）内のサブセクション検出 |
