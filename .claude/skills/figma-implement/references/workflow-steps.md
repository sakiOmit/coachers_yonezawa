...
上記の情報を以下のフォーマットで整理し、**必ずwordpress-professional-engineerエージェントに渡す**:
...
### 6-1. WordPress実装 (wordpress-professional-engineer agent使用)

Task toolでwordpress-professional-engineerを起動:

```markdown
Task tool prompt:
---
以下のFigma Node仕様に基づいて、[ページ名]ページを実装してください。
...
1. ページテンプレート作成: `themes/{{THEME_NAME}}/pages/page-[slug].php`
   - Template Name定義
   - PageHeaderコンポーネント使用
   - パンくず設定
   - FLOCSS + BEM準拠のクラス名（独立Block: `p-[page]-[section]`）
   - **200行以上と推定される場合**: `get_template_part()` 呼び出しのみのメインファイル

2. SCSS作成:
   - `src/scss/object/projects/[page]/` ディレクトリ作成
   - セクション別SCSSファイル作成 (例: `_p-about-hero.scss`, `_p-about-mission.scss`)
   - `src/css/pages/[page]/style.scss` エントリーポイント作成
...
3. ビルド設定:
   - `vite.config.js`にエントリー追加
   - `themes/{{THEME_NAME}}/inc/enqueue.php`に読み込み設定追加

4. **template-parts作成（200行以上の場合は必須）:**
   - `template-parts/[page]/` ディレクトリ作成
   - セクションごとに独立ファイル作成: `hero.php`, `mission.php`, `contact.php` 等
   - 各ファイルは独立した `<section>` タグを持つ
   - SCSSファイル名と完全一致させる
...
開発サーバーが起動したら、ユーザーにローカルURL（例: http://localhost:8000/about/）を確認してもらう。
...
1. Figmaフレーム名: "Frame 123"
   検出内容: テキスト「企業理念」を含む
   → 推測セクション名: "philosophy"
   → WordPressクラス: .p-about__philosophy
   [OK / 別名を入力]
...
for each section:
  mcp__playwright__browser_take_screenshot
    ref: section.wordpressSelector
    type: "png"
    filename: ".claude/cache/visual-diff/wp_{section}.png"
```
...
node scripts/visual-diff.js \
  ...
  .claude/cache/visual-diff/wp_{section}.png \
  --preset default \
...
for each failedSection:
  ...
  while iteration < 5 and not passed:
    1. 差分画像から問題箇所を特定
    2. wordpress-professional-engineer に修正指示
...
SP検証は幅変更後のWordPressページをキャプチャし、PC用Figmaスクリーンショットと比較:

```bash
# SP用の検証（PCのFigmaを参照）
node scripts/visual-diff.js \
  .claude/cache/visual-diff/figma_{section}_pc.png \
  .claude/cache/visual-diff/wp_{section}_sp.png \
  --preset lenient \
